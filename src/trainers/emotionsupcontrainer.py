...
class IemocapLinearTrainer(TrainerBase):
    def __init__(self, args, encoder, model, criterion, optimizer, scheduler, device, dataloaders):
        super(IemocapLinearTrainer, self).__init__(args, model, criterion, optimizer, scheduler, device, dataloaders)
        self.args = args
    ...
    def train_one_epoch(self):
        ...
        for uttranceId, imgs, imgLens, specgrams, specgramLens, text, Y in pbar:
            ...
            with torch.no_grad():
                feature = self.encoder(imgs, imgLens, specgrams, specgramLens, text)

            with torch.set_grad_enabled(True):
                logits = self.model(feature) # (batch_size, num_classes)
                loss = self.criterion(logits, Y)
                loss.backward()
                epoch_loss += loss.item() * Y.size(0)
                data_size += Y.size(0)
